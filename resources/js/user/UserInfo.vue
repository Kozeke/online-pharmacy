    ...
    <div class="personal_info">
      ...
      <div class="detail">
        <div class="photo">
          ...
          <div v-if="do_change" class="change">
            <label for="files" class="btn">Измeнить фото</label>
        ...
        <div class="infos">
          <div class="form-group row">
            ...
            <div class="col-9">
              ...
              <span class="col-form-info" v-if="!do_change">{{user.surname}}</span>
            </div>
          </div>
          <div v-bind:class="{ 'has-error': errors && errors.first_name }" class="form-group row">
            ...
            <div class="col-9">
              ...
              <span
                ...
                :errors="errors"
              >Заполните это поле</span>
              <label class="col-form-info" v-if="!do_change">
                <span>{{user.name}}</span>
              </label>
            </div>
          </div>
          ...
          <div
            v-if="pharm"
            v-bind:class="{ 'has-error': errors && errors.email }"
            class="form-group row"
          >
            <label class="col-3 col-form-label">Номер лицензии</label>
            <div class="col-9">
              <input
                class="form-control"
                :class="{ 'red_border': errors && errors.name}"
                type="text"
                v-model="user.pharms[0].licence_number"
                v-if="do_change"
              >
              <img
                style="width: 16px;height: 16px;"
              ...
              >Заполните это поле</span>
              <label class="col-form-info" v-if="!do_change">
                <span v-model="user.pharms[0].licence_number">{{user.pharms[0].licence_number}}</span>
              </label>
            </div>
          </div>
          <div v-bind:class="{ 'has-error': errors && errors.email }" class="form-group row">
            <label class="col-3 col-form-label">Город</label>
            <div class="col-9">
              <select class="form-control" v-model="user.locality_id" v-if="do_change">
                <option
              ...
              >Заполните это поле</span>
              <label class="col-form-info" v-if="!do_change">
                <span v-model="user.locality_id">{{user.locality.name}}</span>
              </label>
            </div>
          </div>
          <div
            ...
            class="form-group row"
          >
            <label class="col-3 col-form-label">Адрес</label>
            <div class="col-9">
              <input
                class="form-control"
                :class="{ 'red_border': errors && errors.name}"
              ...
              >Заполните это поле</span>
              <label class="col-form-info" v-if="!do_change">
                <span v-model="user.address">{{user.address}}</span>
              </label>
            </div>
          </div>
        </div>
      ...
      <div class="make_change" v-if="do_change">
        ...
        <a class="save" v-on:click="saveChange();do_change=false">Сохранить</a>
      </div>
...
export default {
      ...
      post(
        ...
        _this.pharm
          ? {
              ...
              pharm_name: _this.user.pharms[0].name,
              licence_number: _this.user.pharms[0].licence_number,
              pharm: _this.pharm,
              address: _this.user.address,
              locality_id: _this.user.locality_id,
  ...
  computed: {
    user() {
      return this.$root.user.data ? this.$root.user.data : "";
    },
    changed() {
      return this.$root.user.data ? this.$root.user.data.id : "";
    },
    user_image() {
      return this.$root.user.data
        ? this.$root.user.data.image
          ? this.$root.user.data.image
          : window.location.origin + "/storage/user_photos/default_photo.svg"
        : "";
    },
    pharm() {
      return this.$root.user.data ? this.$root.user.data.isPharmacy : "";
    }
  },
  watch: {
